...
describe('SnowDistribution', () => {
  it('Should inverse bell distribution', () => {
    ...
    const lastThirdIdx = sortedDistributionIdx.slice(
      Math.round((2 * countIdx) / 3),
      Math.round((2 * countIdx) / 3) + 1
    )[0];
    const lastQuarterIdx = sortedDistributionIdx.slice(
      Math.round((3 * countIdx) / 4),
      Math.round((3 * countIdx) / 4) + 1
    )[0];
